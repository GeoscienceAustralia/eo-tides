...
The [`eo_tides.eo`](https://geoscienceaustralia.github.io/eo-tides/api/#eo_tides.eo) module contains the package's core functionality, focusing on tools for attributing `xarray`-format satellite data with modelled tide heights. These tools can be applied to satellite data from any coastal location on the planet, for example using data loaded from the cloud using the [Open Data Cube](https://www.opendatacube.org/) and SpatioTemporal Asset Catalogue [@stac2024].
